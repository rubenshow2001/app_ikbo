...
const fetchAndRenderReport = async () => {
    ...
    try {
        const response = await fetch(`${API_URL}inventario/inventarioxlote`);
    ...
    const renderReportTable = (products) => {
        ...
        products.forEach(product => {
            ...
            if (product.CantidadVencida > 0) {
                //statusClass = 'table-danger';
                statusIcon2 = '<i class="fas fa-exclamation-circle me-1 text-danger"></i> '+product.CantidadVencida;
				statusIcon = '<i class="fas fa-exclamation-triangle me-1 text-danger"></i> Stock vencido';
            } else {
                statusIcon2 = '<i class="fas fa-check-circle me-1 "></i>' + product.CantidadVencida;
            }
